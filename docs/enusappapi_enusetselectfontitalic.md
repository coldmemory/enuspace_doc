...
```cpp
void CenuSpaceView::OnFontItalic()
{
    CMFCRibbonBar* pRibbon = ((CMDIFrameWndEx*) AfxGetMainWnd())->GetRibbonBar(); 
    ASSERT_VALID(pRibbon); 
    CMFCRibbonButton* pButton = DYNAMIC_DOWNCAST(CMFCRibbonButton, pRibbon->FindByID(ID_FONT_ITALIC));
    if (pButton == NULL)
        return;
    ...
    enuSetSelectFontItalic(m_pENUView, !bCheck);
}
```
