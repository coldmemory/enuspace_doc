...
```cpp
void CenuSpaceView::OnFontUnderline()
{
	CMFCRibbonBar* pRibbon = ((CMDIFrameWndEx*) AfxGetMainWnd())->GetRibbonBar(); 
	ASSERT_VALID(pRibbon); 
	CMFCRibbonButton* pButton = DYNAMIC_DOWNCAST(CMFCRibbonButton, pRibbon->FindByID(ID_FONT_UNDERLINE));
	if (pButton == NULL)
		return;
	...
	enuSetSelectFontUnderline(m_pENUView, !bCheck);
}
```
