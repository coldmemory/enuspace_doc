...
![](./assets/3d/faceset.png)

---

face\(삼각형\) 객체에 대하여 설명합니다. enuSpace는 OpenGl엔진을 이용합니다.
...
| coordIndex | string | 좌표데이터의 인덱스를 나열하는 속성 | string |
| normalIndex | string | 일반데이터의 인덱스를 나열하는 속성 | string |
| point | string | faceset의 좌표를 나열하는 속성\(ex: x y z x y z\) | string |
...
```lua
function _ontaskview()
    ID_FACESET.translation_x = 50    -- 객체 내부에서 속성 변경시 직접 변경 할 수 있음
end
```
...
>
    ...
    >
        <Transform
            ...
            translation="0.000000,0.000000,30.000000"
        >
            <Shape
                id="ID_FACESET"
            >
